...
\bottomrule
\end{tabular}
\caption{Convolutional battery (mean over three evaluation seeds): corruption detection on the mixed batch and OOD detection where a counterpart is defined (MNIST CNN vs.\ Fashion-MNIST; Fashion CNN vs.\ MNIST, the subset direction). Base accuracies $99.3\%$, $88.7\%$, $84.5\%$; seed dispersion is at most $\pm 0.004$ for every entry.}
\label{tab:conv}
\end{table}
