...
As in \cref{eq:sol}, the discount operator \( \otimes \) models how trust in an input is modulated by trust in the associated parameter, while \( \bigoplus \) combines these contributions across inputs; fusion operators should be associative or generalizable~\cite{vanderheijden2018multisourcefusionoperationssubjective} to support multiple inputs. Reading the fused terms as opinions of distinct virtual sources about the same node output keeps this mapping algebraically consistent (Appendix~\ref{app:operators} gives the construction); this is precisely the consistency that variable-level fusion, as used in DeepTrust, lacks. Among the fusion variants, the averaging form is used for feedforward summation because it is fan-in invariant: fusing $N$ identically distributed source opinions leaves the scale of the result independent of $N$, so trust magnitudes remain comparable across layer widths and architectures, whereas cumulative fusion accumulates evidence with $N$ and drives wide layers toward dogmatic opinions. Cumulative (evidence-additive) fusion is reserved for pooling genuinely accumulating evidence, as in the input-conformity construction of \cref{exp:4}.
