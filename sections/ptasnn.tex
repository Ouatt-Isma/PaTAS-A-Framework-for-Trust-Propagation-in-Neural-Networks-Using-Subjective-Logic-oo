...
Since all neurons are typically activated to some degree while only a subset is strongly relevant, GenIPTA can also operate on truncated activation sets or weight the assessment by the activation values, so that stronger activations contribute more heavily.

\paragraph*{Convolutional networks} Under weight sharing an activation path cannot select weights, only \emph{applications} of a filter, so binary neuron pruning generalizes to a participation degree: each channel's contribution to the next layer's fusion is weighted by the fraction of spatial positions where its post-ReLU output was positive, and the bias and input rows keep weight one. For binary participation this weighted propagation coincides exactly with the row-pruned propagation of the fully connected case, so conv IPTA is a strict generalization rather than a new mechanism; average pooling is fusion over the window and a residual connection fuses the block input with the block output. \Cref{exp:4} evaluates this per-sample conv IPTA on three convolutional networks. The functional flow of PaTAS across feedforward, backpropagation, and inference is illustrated in Appendix~\ref{app:flow}.
...
For each incoming edge \(j\), the parameter trust is then revised with the deduced neuron trust, \(T_{\theta_{i,j}^{(l)}} \leftarrow T_{\theta_{i,j}^{(l)}} \circleddash T_{n_{i}^{(l)} \parallel Y_\text{batch}}\), and adjusted for the auxiliary factors that also drive the gradient update in \cref{eq:gd1,eq:gd2}, namely the input feature and the batch labels (learning-rate trust is a possible third factor; it is not modeled in this work):
\begin{equation}
