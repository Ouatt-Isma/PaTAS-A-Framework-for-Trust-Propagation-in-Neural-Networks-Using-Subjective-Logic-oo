...
The conservative combination $\oslash$ ($b = \min(b_1, b_2)$, $d = \max(d_1, d_2)$, $u = 1 - b - d$) is not a standard SL operator; its algebraic behavior is elementary: it is commutative, associative, and idempotent (componentwise $\min$ and $\max$ are), and monotone in both arguments (increasing either belief cannot decrease the combined belief, increasing either disbelief cannot decrease the combined disbelief). It is used only to combine the feature and label opinions into the most conservative joint reading before revision.
\begin{table}[H]
\centering
...
Theorem~\ref{thm:geo} analyzes the operator with Jøsang's base-rate dynamics $a_{x \odot y} = a_x a_y$. The implementation instead holds every base rate fixed at $0.5$ through all operators (the convention stated with the experiments), which only strengthens the conclusion: with $a_n = a$ constant, the remainder bound in the proof holds with $a_n$ replaced by the constant, and the same limits follow.
...
\emph{Scope.} The convergence statement covers the revision dynamics: Theorem~\ref{thm:arithm} applies to the $\circleddash$ step with a stabilized revision opinion. Each batch additionally applies the multiplicative adjustment of \cref{eq:layerprop}-style operators (Eq.~(8)); the fixed point and contraction of the composed revise-then-multiply map are not established here and are left open. Empirically the composed iteration is stable (Appendix~\ref{app:expres}); a fixed-point analysis of the composed map is future theoretical work.
...
\begin{algorithmic}[1]
    ...
    \EndFunction
    \Function{Update}{$T_{\theta^{(l)}_{i,j}},
...
For labels, ``complete replacement'' draws a different class uniformly; with two classes this is deterministic flipping, which is why fully distrusted labels yield near-zero rather than chance-level test accuracy in \cref{tab:cancer_results}.
...
\begin{tabular}{lccc}
\toprule
Hidden width & $\epsilon=0.02$ & $\epsilon=0.05$ & $\epsilon=0.1$ \\
\midrule
16 & 0.253 & 0.306 & 0.316 \\
32 & 0.296 & 0.325 & 0.328 \\
64 & 0.317 & 0.333 & 0.334 \\
128 & 0.325 & 0.337 & 0.337 \\
\bottomrule
\end{tabular}
\caption{Width control for \cref{exp:2} (vacuous assessments): converged trust mass across architectures at three gradient-evidence thresholds. The width ordering is identical at every $\epsilon$, ruling out the fixed-threshold confound; the magnitudes are $\epsilon$-dependent, consistent with \cref{tab:abl-eps}.}
\label{tab:abl-width}
\end{table}
